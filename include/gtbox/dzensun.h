...
void gtbox_dzensun_init(GtboxDzenSun *tchant_dzen_sun) {
    tchant_dzen_sun->pool = typool_allocate(GTBOX_DZENSUN_POOLD_,  sizeof(float) * 6);
    tchant_dzen_sun->aims = typool_allocate(GTBOX_DZENSUN_AIMSD_,  sizeof(float) * 2);
...
void gtbox_dzensun_step(
    GtboxDzenSun *gtbox_dzen_sun, 
    void (*callback)(float *, float *,float *, float *, float *)) 
    {
        ...
        if ( 
            ...
            gtbox_dzen_sun->elapsed > GTBOX_DZENSUN_FREC_)
            {
                ...
                *x = *aims_x + d.x * GTBOX_DZENSUN_SUMMONDIST_;
                *y = *aims_y + d.y * GTBOX_DZENSUN_SUMMONDIST_;
                *dx = d.x;
                *dy = d.y;
                *e = 0.0;
                *intensity = 0.0;
